...
//
//    Boost Statechart Viewer is free software: you can redistribute it and/or
...
//    along with Boost Statechart Viewer.  If not, see
...
ostream &operator<<(ostream &os, const State &s) {
  string label = s.name;
  for (list<string>::const_iterator i = s.defferedEvents.begin(),
                                    e = s.defferedEvents.end();
       i != e; ++i)
    label.append("<br />").append(*i).append(" / defer");
  for (list<string>::const_iterator i = s.inStateEvents.begin(),
                                    e = s.inStateEvents.end();
       i != e; ++i)
  ...
  if (s.size()) {
    os << indent << s.name << " -> " << s.initialInnerState
       << " [style = dashed]\n";
...
public:
  ...
  void addUndefinedState(State *m) { undefined[m->name] = m; }
  ...
  void write_as_dot_file(string fn) {
    ...
    for (iterator i = begin(), e = end(); i != e; i++)
      f << i->second;
    for (list<Transition *>::iterator t = transitions.begin(),
                                      e = transitions.end();
         t != e; ++t)
      f << **t;
    f << indent_dec << "}\n";
...
public:
  bool isDerivedFrom(const char *baseStr,
                     CXXBaseSpecifier const **Base = 0) const {
...
public:
  explicit FindTransitVisitor(Model::Model &model,
                              const CXXRecordDecl *SrcState,
                              const Type *EventType)
  ...
  bool VisitMemberExpr(MemberExpr *E) {
    ...
    if (E->hasExplicitTemplateArgs()) {
      ...
      CXXRecordDecl *Event = EventType->getAsCXXRecordDecl();
      Model::Transition *T = new Model::Transition(
          SrcState->getName(), DstState->getName(), Event->getName());
...
class Visitor : public RecursiveASTVisitor<Visitor> {
  ...
  unsigned diag_unhandled_reaction_type, diag_unhandled_reaction_decl,
      ...
      diag_missing_reaction, diag_warning;
  std::vector<bool> reactMethodInReactions; // Indicates whether i-th react
                                            // method is referenced from typedef
                                            // reactions.
...
public:
  ...
  explicit Visitor(ASTContext *Context, Model::Model &model,
                   DiagnosticsEngine &Diags)
      : ASTCtx(Context), model(model), Diags(Diags) {
    diag_found_statemachine = Diags.getCustomDiagID(DiagnosticsEngine::Note,
                                                    "Found statemachine '%0'");
    diag_found_state =
        Diags.getCustomDiagID(DiagnosticsEngine::Note, "Found state '%0'");
    diag_unhandled_reaction_type = Diags.getCustomDiagID(
        DiagnosticsEngine::Error, "Unhandled reaction type '%0'");
    diag_unhandled_reaction_decl = Diags.getCustomDiagID(
        DiagnosticsEngine::Error, "Unhandled reaction decl '%0'");
    diag_no_history = Diags.getCustomDiagID(DiagnosticsEngine::Error,
                                            "History is not yet supported");
    diag_missing_reaction = Diags.getCustomDiagID(
        DiagnosticsEngine::Error, "Missing react method for event '%0'");
    ...
    for (auto it = ReactRes.begin(), end = ReactRes.end(); it != end; ++it, ++i) {
      if (i >= reactMethodInReactions.size() ||
        ...
        Diag(React->getParamDecl(0)->getLocStart(), diag_warning)
            << React->getParamDecl(0)->getType().getAsString()
            << " missing in typedef reactions";
      }
    }
  }

  bool HandleCustomReaction(const CXXRecordDecl *SrcState,
                            const Type *EventType) {
    unsigned i = 0;
    IdentifierInfo &II = ASTCtx->Idents.get("react");
      ...
      if (CXXMethodDecl *React = dyn_cast<CXXMethodDecl>(*it)) {
        if (React->getNumParams() >= 1) {
          ...
          if (ParmType->isLValueReferenceType())
            ParmType = dyn_cast<LValueReferenceType>(ParmType)
                           ->getPointeeType()
                           .getTypePtr();
          if (ParmType == EventType) {
            FindTransitVisitor(model, SrcState, EventType)
                .TraverseStmt(React->getBody());
  ...
  void HandleReaction(const Type *T, const SourceLocation Loc,
                      CXXRecordDecl *SrcState) {
    // TODO: Improve Loc tracking
    if (const ElaboratedType *ET = dyn_cast<ElaboratedType>(T))
      HandleReaction(ET->getNamedType().getTypePtr(), Loc, SrcState);
    else if (const TemplateSpecializationType *TST =
                 dyn_cast<TemplateSpecializationType>(T)) {
      string name = TST->getTemplateName()
                        .getAsTemplateDecl()
      ...
      if (name == "boost::statechart::transition") {
        ...
        if (!HandleCustomReaction(SrcState, EventType)) {
          Diag(SrcState->getLocation(), diag_missing_reaction)
              << EventType->getAsCXXRecordDecl()->getName();
        }
        unusedEvents.remove_if(
            eventHasName(EventType->getAsCXXRecordDecl()->getNameAsString()));
        ...
        for (TemplateSpecializationType::iterator Arg = TST->begin(),
                                                  End = TST->end();
             Arg != End; ++Arg)
  ...
  void HandleReaction(const NamedDecl *Decl, CXXRecordDecl *SrcState) {
    if (const TypedefDecl *r = dyn_cast<TypedefDecl>(Decl))
      ...
      Diag(Decl->getLocation(), diag_unhandled_reaction_decl)
  ...
  TemplateArgumentLoc getTemplateArgLoc(const TypeLoc &T, unsigned ArgNum,
    ...
    else if (const TemplateSpecializationTypeLoc TST =
                 T.getAs<TemplateSpecializationTypeLoc>()) {
      if (TST.getNumArgs() >= ArgNum + 1) {
        return TST.getArgLoc(ArgNum);
      } else if (!ignore)
        Diag(TST.getBeginLoc(), diag_warning)
            << TST.getType()->getTypeClassName() << "has not enough arguments"
            << TST.getSourceRange();
    } else
      Diag(T.getBeginLoc(), diag_warning)
          << T.getType()->getTypeClassName()
          << "type as template argument is not supported" << T.getSourceRange();
    return TemplateArgumentLoc();
  }

  TemplateArgumentLoc getTemplateArgLocOfBase(const CXXBaseSpecifier *Base,
                                              unsigned ArgNum, bool ignore) {
    return getTemplateArgLoc(Base->getTypeSourceInfo()->getTypeLoc(), ArgNum,
                             ignore);
  }

  CXXRecordDecl *getTemplateArgDeclOfBase(const CXXBaseSpecifier *Base,
                                          unsigned ArgNum,
    ...
    case TemplateArgument::Null:
      ...
      break;
    default:
      Diag(Loc.getSourceRange().getBegin(), diag_warning)
          << Loc.getArgument().getKind() << "unsupported kind"
          << Loc.getSourceRange();
    }
    return 0;
  }
  ...
  void handleSimpleState(CXXRecordDecl *RecordDecl,
    ...
    TemplateArgumentLoc Loc;
    if (MyCXXRecordDecl *InnerInitialState = static_cast<MyCXXRecordDecl *>(
            getTemplateArgDeclOfBase(Base, 2, Loc, true))) {
      if (InnerInitialState->isDerivedFrom("boost::statechart::simple_state") ||
          InnerInitialState->isDerivedFrom(
              "boost::statechart::state_machine")) {
        state->setInitialInnerState(InnerInitialState->getName());
      } else if (!InnerInitialState->getNameAsString().compare(
                     "boost::mpl::list<>"))
        Diag(Loc.getTypeSourceInfo()->getTypeLoc().getBeginLoc(), diag_warning)
            << InnerInitialState->getName()
            << " as inner initial state is not supported"
            << Loc.getSourceRange();
    }

    //      if (CXXRecordDecl *History =
    //      getTemplateArgDecl(Base->getType().getTypePtr(), 3))
    //      Diag(History->getLocStart(), diag_no_history);

    IdentifierInfo &II = ASTCtx->Idents.get("reactions");
    // TODO: Lookup for reactions even in base classes - probably by using
    // Sema::LookupQualifiedName()
  ...
  void handleStateMachine(CXXRecordDecl *RecordDecl,
                          const CXXBaseSpecifier *Base) {
  ...
  bool VisitCXXRecordDecl(CXXRecordDecl *Declaration) {
    ...
    if (Declaration->getQualifiedNameAsString() == "boost::statechart::state" ||
        ...
        Declaration->getQualifiedNameAsString() ==
            "boost::statechart::assynchronous_state_machine")
      return true; // This is an "abstract class" not a real state or real state
                   // machine
    ...
    if (RecordDecl->isDerivedFrom("boost::statechart::simple_state", &Base))
      ...
      unusedEvents.push_back(
          eventModel(RecordDecl->getNameAsString(), RecordDecl->getLocation()));
    }
    return true;
  }
  void printUnusedEventDefinitions() {
    for (list<eventModel>::iterator it = unusedEvents.begin();
         it != unusedEvents.end(); it++)
...
public:
  explicit VisualizeStatechartConsumer(ASTContext *Context,
...
protected:
  unique_ptr<ASTConsumer> CreateASTConsumer(CompilerInstance &CI, llvm::StringRef) {
    size_t dot = getCurrentFile().find_last_of('.');
    string dest = getCurrentFile().substr(0, dot);
    dest.append(".dot");
    return llvm::make_unique<VisualizeStatechartConsumer>(&CI.getASTContext(), dest,
                                           CI.getDiagnostics());
  }
...
static FrontendPluginRegistry::Add<VisualizeStatechartAction>
    X("visualize-statechart", "visualize statechart");
